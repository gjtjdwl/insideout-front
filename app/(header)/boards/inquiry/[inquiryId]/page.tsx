...
const BoardDetail = ({ params }: Props) => {
  ...
  const inquiryDetail = async (inquiryId: number): Promise<void> => {
    try {
      const res = await API.get<InquiryData, any>(`/api/boards/inquiry/${inquiryId}`);
      //유저아이디 추가되면 게시물 유저아이디랑 내 아이디랑 비교해서 보기권한 설정하기 

      setDetail(res.data);
      const formattedTime = formatDateTime(res.data.createdTime);
      ...
      <div className="flex flex-col items-start">
        <div className="border-b w-full">
          <div className="px-4 py-2">
            <div>
              <div>
                <span className="lg:text-xl font-bold mr-2"> { detail.title } </span>
